...
<ul class="nav nav-tabs" id="myTab" role="tablist">
    @for ($i = 0; $i < count($pekerjaan); $i++) <li class="nav-item " role="presentation">
...
<div class="tab-content" id="myTabContent">
    ...
    <div class="tab-pane fade {{$loop->iteration == count($pekerjaan)? 'show active':''}} " id="pekerjaan{{$loop->iteration}}-tab-pane" role="tabpanel" aria-labelledby="pekerjaan{{$loop->iteration }}-tab" tabindex="0">
        ...
        <div class="col-sm-4 bg-light p-4 rounded">
            ...
            <label class="text-secondary" for="">Nama Tempat Bekerja</label>
            <h6 class="fw-bold">{{$work->tempat_pekerjaan}}</h6>

            <label class="text-secondary" for="">Alamat Bekerja</label>
            <h6 class="fw-bold">Kelurahan {{ucfirst(strtolower($work->kelurahan))}} Kecamatan {{ucfirst(strtolower($work->kecamatan))}} {{ucfirst(strtolower($work->kabupaten))}} Provinsi {{ucfirst(strtolower($work->provinsi))}}</h6>
            ...
            <h6 class="mt-5">
                <form action="/alumni/works/{{$work->id}}" method="post">
                    ...
                    @method('delete')
                    @csrf
                    <button class="btn btn-sm btn-outline-danger ms-3">Hapus</a>
            </h6>
            </form>
        </div>
