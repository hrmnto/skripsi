...
@section('container')
    ...
    <h1 class="h2 text-primary mb-3">Tabel Pekerjaan</h1>
        ...
        <div class="card-body">
            <div class="d-flex justify-content-end flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
                <div class="">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('pekerjaan-import') }}" method="post"
                    ...
                    <a href="{{ route('pekerjaan-export') }}" class="text-primary btn btn-sm"><svg
                ...
                <tbody>
                    ...
                    @foreach ($pekerjaans as $pekerjaan)
                        <tr>
                            ...
                            <td>{{ $pekerjaan->nim }}</td>
                            <td>{{ $pekerjaan->biodata->name ?? 'Tidak ada nama' }}</td>
                            <td>{{ $pekerjaan->kategori_pekerjaan1 == 1 ? 'Kependidikan ' : '' }}{{ $pekerjaan->kategori_pekerjaan2 == 1 ? 'IT ' : '' }}
                                {{ $pekerjaan->kategori_pekerjaan3 == 1 ? 'Wirausaha' : '' }}</td>
                            <td>{{ $pekerjaan->nama_pekerjaan }}</td>
                            <td>{{ $pekerjaan->biodata && $pekerjaan->biodata->tglLulus ? date('d F Y', strtotime($pekerjaan->biodata->tglLulus)) : 'Belum ada tanggal' }}
                            </td>
                            @php
                                $tgl2 = new DateTime($pekerjaan->tanggal_pekerjaan);
                                $tgl1 = $pekerjaan->biodata && $pekerjaan->biodata->tglLulus ? new DateTime($pekerjaan->biodata->tglLulus) : null;
                                ...
                                if ($tgl1) {
                                    ...
                                    $str = 'Data tanggal tidak tersedia';
                                }
                            ...
                            <td>
                                <form action="/admin/pekerjaan/{{ $pekerjaan->id }}" method="post">
                                    @method('delete')
                                    @csrf
                                    @if ($pekerjaan->biodata && $pekerjaan->biodata->name != 'admin')
                                        <button class="btn btn-sm text-danger"><svg xmlns="http://www.w3.org/2000/svg"
